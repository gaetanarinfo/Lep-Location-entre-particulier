...
if (isset($_POST)) {
    ...
    if (empty($donnees['email'])) {
        ...
        $update = $dbh->query('UPDATE `users` SET `nom` = "' . $_POST['last_name'] .'", `prenom` = "' . $_POST['first_name'] .'", `id_facebook` = "' . $_POST['id'] . '", `avatar` = "' . $_POST['avatar'] . '", `last_online` = "' . date('Y-m-d H:i:s') . '", `ip` = "' . $_SERVER['REMOTE_ADDR'] . '" WHERE email = "' . $email . '"');
