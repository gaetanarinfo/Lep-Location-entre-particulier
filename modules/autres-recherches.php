        ...
        <div class="collapseTable">
            ...
            <div class="collaspeRow">
                ...
                foreach ($locationsCols1_last_row as $value) {
                    ...
                    $regionCounterCols1 = $dbh->query('SELECT COUNT(id) FROM appartements WHERE id_site = 1 AND region = ' . $value['id']);
                    $count = $regionCounterCols1->fetchColumn();

                    // Je veux uniquement le nom de la ville pour chaque annonce
                    $villeRow1 = $dbh->query('SELECT location FROM appartements WHERE id_site = 1 AND region = ' . $value['id']);
            ...
            <div class="collaspeRow">
                ...
                foreach ($locationsCols2_last_row as $value) {
                    ...
                    // On compte le nombre de résultat par Région
                    $regionCounterCols2 = $dbh->query('SELECT COUNT(*) FROM appartements WHERE id_site = 1 AND region = ' . $value['id']);
                    $count = $regionCounterCols2->fetchColumn();

                    // Je veux uniquement le nom de la ville pour chaque annonce
                    $villeRow2 = $dbh->query('SELECT location FROM appartements WHERE id_site = 1 AND region = ' . $value['id']);
